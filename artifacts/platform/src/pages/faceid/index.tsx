...
import { Loader2, Play, ScanFace, Send, ShieldCheck, UserCheck, Clock, Users, Settings2 } from "lucide-react";
...
interface Overview {
  date: string;
  time: string;
  settings: { late_after: string; notify: boolean; threshold: number };
  students: number;
  enrolled: number;
  arrived: number;
  late: number;
  classes: { class_name: string; total: number; enrolled: number; arrived: number; late: number }[];
  recent: { student_name: string; class_name: string; status: string; time: string }[];
}
...
export default function FaceIdPage() {
  ...
  }, [ov.data?.settings.late_after, ov.data?.settings.notify, ov.data?.settings.threshold]); // eslint-disable-line react-hooks/exhaustive-deps

  const saveSettings = async () => {
    setSaving(true);
    try {
      await api("/faceid/settings", { method: "POST", body: JSON.stringify({ late_after: lateAfter, notify, threshold: Number(threshold) }) });
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Maktabga kirishda yuz orqali avtomatik davomat — oddiy telefon kamerasi bilan</p>
        </div>
      ...
      {d && (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            ...
            <Stat icon={ScanFace} label="Face ID'da ro'yxatda" value={d.enrolled} sub={`${d.students} o'quvchidan`} />
            <Stat icon={Users} label="Sinflar" value={d.classes.length} sub={`Yangilandi: ${d.time}`} />
          </div>
          ...
          <div className="grid lg:grid-cols-3 gap-4">
                ...
                {d.classes.map((c) => {
                  ...
                  return (
                    <div key={c.class_name} className="flex items-center gap-3">
                      ...
                      <div className="w-36 text-right text-xs text-muted-foreground tabular-nums">
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-base">Oxirgi kelganlar</CardTitle></CardHeader>
              <CardContent className="space-y-1.5">
                {d.recent.map((r, i) => (
                  <div key={i} className="flex items-center justify-between text-sm">
                    <span className="truncate"><span className={r.status === "late" ? "text-amber-500" : "text-emerald-500"}>●</span> {r.student_name}</span>
                    <span className="text-muted-foreground text-xs shrink-0 ml-2">{r.class_name} · {r.time}</span>
                  </div>
                ))}
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  ...
                  O'quvchiga Telegramda "Maktabga keldingiz" xabari
                </label>

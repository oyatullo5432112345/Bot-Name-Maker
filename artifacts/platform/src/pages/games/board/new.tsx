...
type QuestionVariant = "choice" | "open";
...
const emptyCell = (): DraftCell => ({
  type: "question", question: "", variant: "choice", options: ["", "", "", ""], correct_index: 0, correct_text: "",
...
Javob: A`;
  ...
  const handleParseRawText = () => {
    ...
    setCells(prevCells => {
      ...
      for (const block of blocks) {
        ...
        // Aks holda — bu oddiy savol bloki (variantli yoki variantsiz bo'lishi mumkin)
        ...
        lines.slice(1).forEach(line => {
          const letterAnsMatch = line.match(/^(?:javob|javobi|ans|correct)\s*:?\s*([a-d1-4])\s*$/i);
          if (letterAnsMatch) {
            const val = letterAnsMatch[1]!.toLowerCase();
          ...
          const freeAnsMatch = line.match(/^(?:javob|javobi|ans|correct)\s*:\s*(.+)$/i);
          if (freeAnsMatch) {
            correctText = freeAnsMatch[1]!.trim();
            return;
          }
        ...
        nextCells[ci] = {
          ...emptyCell(),
          type: "question",
          variant: isOpen ? "open" : "choice",
          question: questionText,
          options: isOpen ? ["", "", "", ""] : options.slice(0, 4),
          correct_index: isOpen ? 0 : Math.min(correctIndex, 3),
          correct_text: isOpen ? correctText : "",
        };
        ci++;
      }
      return nextCells;
  ...
  const handleImportFile = async (file: File | null) => {
    ...
    try {
      ...
      setCells(cs => cs.map(c => {
        ...
        const q = imported[qi++]!;
        return { ...c, question: q.question, variant: "choice", options: q.options, correct_index: q.correct_index, difficulty: q.difficulty, points: DEFAULT_POINTS[q.difficulty] };
      }));
  ...
  const handleSave = async () => {
    ...
    const invalidQuestion = cells.find(c => c.type === "question" && (
      !c.question.trim() || (c.variant === "choice" ? c.options.some(o => !o.trim()) : !c.correct_text.trim())
    ));
    if (invalidQuestion) { toast({ variant: "destructive", title: "To'ldirilmagan savol bor", description: "Har bir savol katakchasida matn, va variantli bo'lsa barcha variantlar, variantsiz bo'lsa to'g'ri javob to'ldirilishi kerak" }); return; }
      ...
      const r = await fetch(`${API_BASE}/board-games`, {
        ...
        body: JSON.stringify({
          ...
          cells: cells.map((c, i) => ({
            ...
            ...(c.type === "question" ? {
              question: c.question, difficulty: c.difficulty, points: DEFAULT_POINTS[c.difficulty],
              ...(c.variant === "choice" ? { options: c.options, correct_index: c.correct_index } : { correct_text: c.correct_text }),
            } : {}),
              ...
              {c.type === "question" && (
                <div className="pl-9 space-y-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => updateCell(i, { variant: "choice" })}
                      className={`text-[11px] px-2.5 py-0.5 rounded-full border font-medium ${c.variant === "choice" ? "bg-primary/10 border-primary text-primary" : "border-border text-muted-foreground"}`}
                    >
                      Variantli (4 ta javob)
                    </button>
                    <button
                      onClick={() => updateCell(i, { variant: "open" })}
                      className={`text-[11px] px-2.5 py-0.5 rounded-full border font-medium ${c.variant === "open" ? "bg-primary/10 border-primary text-primary" : "border-border text-muted-foreground"}`}
                    >
                      Variantsiz (og'zaki javob)
                    </button>
                  </div>
                  ...
                  {c.variant === "choice" ? (
                    ...
                    <Input placeholder="To'g'ri javob (namuna, o'qituvchi uchun)" value={c.correct_text} onChange={e => updateCell(i, { correct_text: e.target.value })} className="h-8 text-xs" />
                  )}
        ...
        <DialogContent className="max-w-lg space-y-4">
          ...
          <div className="space-y-3 text-xs">
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                rows={12}
                placeholder={"1. O'zbekiston poytaxti qaysi?\na) Samarqand\nb) Toshkent\nc) Buxoro\nd) Xiva\nJavob: B\n\n2. Bu qanday hayvon? (rasmga qarab ayting)\nJavob: Sher\n\nBONUS: 20\n\nJARIMA: 15\n\nYUTQAZISH\n\nOG'IRLASH: 30"}
                value={rawText}
                onChange={e => setRawText(e.target.value)}
                className="font-mono text-xs"
              />
              <p className="text-[11px] text-muted-foreground font-semibold leading-relaxed">
                Variantli savolni yuqoridagi 1-4 formatida yozing. <span className="font-mono">a) b) c) d)</span> variantlarsiz, faqat savol va <span className="font-mono">Javob: matn</span> yozsangiz — variantsiz (og'zaki javob) savol sifatida qo'shiladi.
                Boshqa turdagi katakchalarni ham shu yerdan qo'shishingiz mumkin — alohida qatorga:

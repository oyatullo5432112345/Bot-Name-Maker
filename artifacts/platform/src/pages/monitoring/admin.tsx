...
import { Plus, Edit2, Trash2, FileText, Loader2, Sparkles, Lock, Unlock, Eye } from "lucide-react";
...
export default function MonitoringAdminPage() {
    ...
    for (const block of blocks) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (/^(javob|ans|correct):/i.test(line)) {
          ...
          if (ansChar) {
            const code = ansChar.charCodeAt(0) - 65;
  ...
  const toggleLock = async (testId: string, currentStatus?: boolean) => {
    try {
      const r = await fetch(`${API_BASE}/monitoring/tests/${testId}/toggle-lock`, {
        method: "PATCH",
        headers: authH(),
        body: JSON.stringify({ is_locked: !currentStatus }),
      });
      if (r.ok) {
        toast({ title: !currentStatus ? "Test qulflandi" : "Test ochildi" });
        qc.invalidateQueries({ queryKey: ["admin-monitoring-tests"] });
      }
    } catch {
      toast({ title: "Xatolik", description: "Statusni o'zgartirishda muammo", variant: "destructive" });
    }
  };

  const fetchResults = async (testId: string) => {
    try {
      const r = await fetch(`${API_BASE}/monitoring/tests/${testId}/results`, { headers: authH() });
      if (r.ok) {
      ...
      toast({ title: "Xatolik", description: "Natijalarni yuklab bo'lmadi", variant: "destructive" });
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6 p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Testlar yaratish, qulflash, tahrirlash va natijalarni ko'rish</p>
        </div>
          ...
          {tests.map((t) => (
            <Card key={t.id} className="relative group">
              <CardHeader className="pb-2 flex flex-row items-center justify-between">
                ...
                <Button variant="ghost" size="icon" onClick={() => toggleLock(t.id, t.is_locked)}>
                  {t.is_locked ? <Lock className="w-4 h-4 text-red-500" /> : <Unlock className="w-4 h-4 text-emerald-500" />}
                </Button>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="text-xs text-muted-foreground space-y-1">
                  <p>Savollar: <b>{t.questions?.length || 0} ta</b> | Vaqt: <b>{t.duration_minutes}m</b></p>
                  <p>Holat: <b>{t.is_locked ? "Qulflangan" : "Ochiq"}</b></p>
                </div>
                <div className="flex gap-2 pt-2 border-t">
                  <Button variant="outline" size="sm" className="gap-1 flex-1" onClick={() => fetchResults(t.id)}>
        ...
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-3 py-2">
            {selectedTestResults.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-4">Hali hech kim javob topshirmagan.</p>
            ) : (
              selectedTestResults.map((r, i) => (
                <div key={i} className="flex justify-between items-center p-3 border rounded-lg text-sm">
                  <div>
                    <p className="font-bold">{r.user_name || "Noma'lum"}</p>
                    <p className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleString()}</p>
                  </div>
                  <p className="font-bold text-primary">{r.score} / {r.total_questions} ball</p>
                </div>
              ))
            )}
          </div>

...
import {
  ...
  FileSpreadsheet, Wallet, ChevronRight, Settings, ClipboardCheck,
} from "lucide-react";
  ...
  return (
    <div className={`flex items-center gap-${isSmall ? "1.5" : "2"} select-none`}>
      ...
      <div>
        ...
        {!isSmall && <div className="text-[9px] text-sidebar-foreground/50 leading-none mt-0.5">3-Maktab</div>}
      </div>
    </div>
  );
}
...
export function AppLayout({ children }: { children: React.ReactNode }) {
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-sidebar-border">
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={toggleTheme}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-sidebar-foreground/70 hover:text-sidebar-accent-foreground hover:bg-sidebar-accent transition-colors"
          >
            {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
          </button>
          <button
            onClick={() => setDrawerOpen(false)}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-sidebar-foreground/70 hover:text-sidebar-accent-foreground hover:bg-sidebar-accent transition-colors lg:hidden"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        ...
        {!isMudir && (
          <NavSection label="Ta'lim">
            ...
            <NavLink
              href={isStudent ? "/monitoring" : "/monitoring/admin"}
              icon={ClipboardCheck}
              label="Chorak Monitoring"
        ...
        <NavSection>
          <NavLink href="/reyting-markazi" icon={Trophy} label="Reyting Markazi" active={isActive("/reyting-markazi") || isActive("/reyting") || isActive("/tanga")} />
        </NavSection>
      ...
      <div className="border-t border-sidebar-border px-3 py-3 space-y-2">
          ...
          <Link href="/tanga">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-sidebar-foreground/60">{tanga.total} 🪙 tanga</p>
              </div>
        ...
        <div className="flex items-center gap-3 px-2 py-2 rounded-lg bg-sidebar-accent">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold truncate text-sidebar-accent-foreground">{user.full_name}</p>
            <p className="text-xs text-sidebar-foreground/60 truncate">{roleDisplay[user.role] || user.role}</p>
            {user.class_name && <p className="text-xs text-sidebar-foreground/60">{user.class_name} sinf</p>}
          </div>
        </div>
        <Button
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex w-64 border-r border-sidebar-border bg-sidebar text-sidebar-foreground flex-col shrink-0">
        {sidebarContent}
      </aside>
      ...
      {/* Mobile drawer panel */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-sidebar text-sidebar-foreground border-r border-sidebar-border shadow-xl flex flex-col transition-transform duration-300 lg:hidden ${
